...
#define CNTACK 5 //累计接收多少个报文回复ACK报文
const int maxsend=10;//最多发送次数

//client端连续收到多少个错误报文才会重发
#define CNTERR 3

bool server_init(WSADATA &wsa,SOCKET &ser_socket,sockaddr_in &addr,sockaddr_in &r_addr){
    cout<< "[UDP可靠传输协议part2--服务器端 by 2211290姚知言] \n";
...
void recv_file(SOCKET ser_socket,sockaddr_in r_addr){
    int r_addrsize=sizeof(r_addr);
    while(true){
        if(recvfin) return;
        if(sendack>0){
...
bool server_recvfile(SOCKET ser_socket,sockaddr_in r_addr){
    ...
    int err=0;
    thread th1(recv_file,ser_socket,r_addr);
    th1.detach();
    while(true){
        if(recvfrom(ser_socket, (char*)&rmsg, sizeof(rmsg), 0, (sockaddr*)&r_addr, &r_addrsize)>0){
            rmsg.printdetails(false);
            if((rmsg.flags == 0) && rmsg.verifychecksum() && rmsg.ack == smsg.seq && rmsg.seq==nowseq){//想要的就是ta
                cout<<"\033[34m 收到了新的文件报文！\033[0m \n";
                err=0;
                smsg.ack = rmsg.seq+rmsg.len;
                nowseq=smsg.ack;
                cntrecv++;
                if(cntrecv == CNTACK || rmsg.seq+rmsg.len == totseq){//达到了累计确认数量，或者是最后一个
                    sendack=1;
                    cntrecv = 0;//清零累计接收数量
                }
                //store
                ...
                cout<<"接收到了信息，但未能通过验证。\n";
                err++;
                if(err <= CNTERR || err % CNTACK == CNTERR){//错误报文小于等于CNTERR，快速返回，之后CNTACK循环
                    if(sendcnt++ >= maxsend){
