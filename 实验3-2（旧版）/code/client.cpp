...
void resend_file(SOCKET u_socket,sockaddr_in r_addr,message* msgbuf){
    int r_addrsize=sizeof(r_addr);
    while(true){
        if(finfile) break;
        if(useresend){
...
bool client_resend(SOCKET u_socket,sockaddr_in r_addr,message* msgbuf,u_long batch){
    ...
    thread resend_thread (resend_file,u_socket,r_addr,msgbuf);
